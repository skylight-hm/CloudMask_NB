...
def get_nav_path(month: str):
    data_root_dir = os.getenv('METEPY_DATA_PATH', 'assets')
...
class FY4NavFile(object):
    ...
    def prepare_surface_type_to_cspp(self, space_mask=False) -> np.ndarray:
        ...
        desert_mask = nav_f['pixel_desert_mask'][
            ...]  # two way :1. CSPP(emiss) 2. fy4
        nb_sft = np.zeros(land_mask.shape, np.uint8)

        # DeepOcean
        nb_sft[land_mask == self.
               ET_Land_class['DEEP_OCEAN']] = self.ET_SFT_class['DeepOcean']
        # ShallowOcean
        nb_sft[land_mask == self.ET_Land_class[
            'MODERATE_OCEAN']] = self.ET_SFT_class['ShallowOcean']
        nb_sft[land_mask == self.ET_Land_class[
            'DEEP_INLAND_WATER']] = self.ET_SFT_class['ShallowOcean']
        nb_sft[land_mask == self.ET_Land_class[
            'SHALLOW_INLAND_WATER']] = self.ET_SFT_class['ShallowOcean']
        nb_sft[
            land_mask == self.
            ET_Land_class['SHALLOW_OCEAN']] = self.ET_SFT_class['ShallowOcean']
        # UnfrozenLand
        nb_sft[land_mask ==
               self.ET_Land_class['LAND']] = self.ET_SFT_class['UnfrozenLand']
        nb_sft[land_mask == self.
               ET_Land_class['COASTLINE']] = self.ET_SFT_class['UnfrozenLand']
        nb_sft[land_mask == self.ET_Land_class[
            'EPHEMERAL_WATER']] = self.ET_SFT_class['UnfrozenLand']
        # SnowLand
        sl_idx = np.logical_and(lat > -60,
                                snow_mask == self.ET_Snow_class['SNOW'])
        nb_sft[sl_idx] = self.ET_SFT_class['SnowLand']
        # Arctic
        arc_idx = np.logical_and(lat >= 0,
                                 snow_mask == self.ET_Snow_class['SEA_ICE'])
        nb_sft[arc_idx] = self.ET_SFT_class['Arctic']
        # Antarctic # no greenland
        ant_idx1 = np.logical_and(lat <= -60,
                                  snow_mask == self.ET_Snow_class['SNOW'])
        ant_idx2 = np.logical_and(lat <= -60,
                                  snow_mask == self.ET_Snow_class['SEA_ICE'])
        nb_sft[ant_idx1] = self.ET_SFT_class['Antarctic']
        nb_sft[ant_idx2] = self.ET_SFT_class['Antarctic']
        # Desert # fy4 way
        desert_idx = np.logical_or(
            desert_mask == self.ET_Desert_class['NIR_DESERT'],
