...
from metesatpy.algorithms.CloudMask import T11
...
t11 = T11()
t11_lut_ds_x = copy.deepcopy(t11.lut_ds)

for idx, csft in enumerate(t11_lut_ds_x.cspp_sft.data.tolist()):
    sft_cursor = idx + 1
    ...
    cloudy_da_fea_total.to_hdf5('%s_60.h5' % t11.short_name, '/%s/cloudy' % csft)

    clear_da_fea_total.to_hdf5('%s_60.h5' % t11.short_name, '/%s/clear' % csft)
    ...
    t11_lut_ds_x.prior_yes.data[idx] = prior_yes
    t11_lut_ds_x.bin_start.data[idx] = bins1[0]
    t11_lut_ds_x.bin_end.data[idx] = bins1[-1]
    t11_lut_ds_x.delta_bin.data[idx] = bins1[1] - bins1[0]
    t11_lut_ds_x.bins.data[idx, :] = bins1[1:]
    t11_lut_ds_x.class_cond_ratio_reg.data[idx, :] = ratio

t11_lut_ds_x.to_netcdf(r'D:\WorkSpace\20200429\project\data\LUT\T_11_60.nc')
