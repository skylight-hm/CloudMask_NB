...
from metesatpy.algorithms.CloudMask import TStd
...
for idx, csft in enumerate(tstd_lut_ds_x.cspp_sft.data.tolist()):
    ...
    cloudy_da_fea_total.to_hdf5('%s_60.h5' % tstd.short_name, '/%s/cloudy' % csft)

    clear_da_fea_total.to_hdf5('%s_60.h5' % tstd.short_name, '/%s/clear' % csft)
    ...
    tstd_lut_ds_x.prior_yes.data[idx] = prior_yes
    tstd_lut_ds_x.bin_start.data[idx] = bins1[0]
    tstd_lut_ds_x.bin_end.data[idx] = bins1[-1]
    tstd_lut_ds_x.delta_bin.data[idx] = bins1[1] - bins1[0]
    tstd_lut_ds_x.bins.data[idx, :] = bins1[1:]
    tstd_lut_ds_x.class_cond_ratio_reg.data[idx, :] = ratio

tstd_lut_ds_x.to_netcdf(r'D:\WorkSpace\20200429\project\data\LUT\T_Std_60.nc')
