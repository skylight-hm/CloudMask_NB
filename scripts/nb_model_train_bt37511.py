...
from metesatpy.algorithms.CloudMask import Btd37511Night
...
bt37511 = Btd37511Night()
bt37511_lut_ds_x = copy.deepcopy(bt37511.lut_ds)

for idx, csft in enumerate(bt37511_lut_ds_x.cspp_sft.data.tolist()):
    sft_cursor = idx + 1
    ...
    for i in tqdm.trange(start_idx, 100, desc='%s' % csft):  # ignore 01 01
        ...
        bt_375 = l1.get_band_by_channel('bt_372_low')
        bt_1080 = l1.get_band_by_channel('bt_1080')

        sun_zen = geo.get_sun_zenith()

        bt37511 = Btd37511Night()
        x = bt37511.prepare_feature(bt_375, bt_1080)
        valid_mask = bt37511.prepare_valid_mask(bt_375, bt_1080, sft, sun_zen, space_mask)
    ...
    cloudy_da_fea_total.to_hdf5('%s_60.h5' % bt37511.short_name, '/%s/cloudy' % csft)

    clear_da_fea_total.to_hdf5('%s_60.h5' % bt37511.short_name, '/%s/clear' % csft)
    ...
    bt37511_lut_ds_x.prior_yes.data[idx] = prior_yes
    bt37511_lut_ds_x.bin_start.data[idx] = bins1[0]
    bt37511_lut_ds_x.bin_end.data[idx] = bins1[-1]
    bt37511_lut_ds_x.delta_bin.data[idx] = bins1[1] - bins1[0]
    bt37511_lut_ds_x.bins.data[idx, :] = bins1[1:]
    bt37511_lut_ds_x.class_cond_ratio_reg.data[idx, :] = ratio

bt37511_lut_ds_x.to_netcdf(r'D:\WorkSpace\20200429\project\data\LUT\Btd_375_11_Night_60.nc')
