...
from metesatpy.algorithms.CloudMask import NdsiDay
...
ndsi = NdsiDay()
ndsi_lut_ds_x = copy.deepcopy(ndsi.lut_ds)

for idx, csft in enumerate(ndsi_lut_ds_x.cspp_sft.data.tolist()):
    sft_cursor = idx + 1
    ...
    for i in tqdm.trange(start_idx, 100, desc='%s' % csft):  # ignore 01 01
        ...
        ref_161 = l1.get_band_by_channel('ref_161') * 100
        ...
        valid_mask = ndsi.prepare_valid_mask(ref_065, ref_161, sft, sun_glint, sun_zen, scat_ang, air_mass, space_mask,
                                             bt_1080)
    ...
    cloudy_da_fea_total.to_hdf5('%s_60.h5' % ndsi.short_name, '/%s/cloudy' % csft)

    clear_da_fea_total.to_hdf5('%s_60.h5' % ndsi.short_name, '/%s/clear' % csft)
    ...
    ndsi_lut_ds_x.prior_yes.data[idx] = prior_yes
    ndsi_lut_ds_x.bin_start.data[idx] = bins1[0]
    ndsi_lut_ds_x.bin_end.data[idx] = bins1[-1]
    ndsi_lut_ds_x.delta_bin.data[idx] = bins1[1] - bins1[0]
    ndsi_lut_ds_x.bins.data[idx, :] = bins1[1:]
    ndsi_lut_ds_x.class_cond_ratio_reg.data[idx, :] = ratio

ndsi_lut_ds_x.to_netcdf(r'D:\WorkSpace\20200429\project\data\LUT\Ndsi_Day_60.nc')
