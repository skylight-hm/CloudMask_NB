...
from metesatpy.algorithms.CloudMask import TmaxT
...
tmax_t = TmaxT()
tmax_t_lut_ds_x = copy.deepcopy(tmax_t.lut_ds)

for idx, csft in enumerate(tmax_t_lut_ds_x.cspp_sft.data.tolist()):
    sft_cursor = idx + 1
    ...
    tmax_t_lut_ds_x.prior_yes.data[idx] = prior_yes
    tmax_t_lut_ds_x.bin_start.data[idx] = bins1[0]
    tmax_t_lut_ds_x.bin_end.data[idx] = bins1[-1]
    tmax_t_lut_ds_x.delta_bin.data[idx] = bins1[1] - bins1[0]
    tmax_t_lut_ds_x.bins.data[idx, :] = bins1[1:]
    tmax_t_lut_ds_x.class_cond_ratio_reg.data[idx, :] = ratio

tmax_t_lut_ds_x.to_netcdf(r'D:\WorkSpace\20200429\project\data\LUT\Tmax_T_60.nc')
