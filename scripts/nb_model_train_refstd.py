...
from metesatpy.algorithms.CloudMask import RefStd
...
ref_std = RefStd()
ref_std_lut_ds_x = copy.deepcopy(ref_std.lut_ds)

for idx, csft in enumerate(ref_std_lut_ds_x.cspp_sft.data.tolist()):
    sft_cursor = idx + 1
    ...
    for i in tqdm.trange(start_idx, 100, desc='%s' % csft):
        ...
        ref_std = RefStd()
        x = ref_std.prepare_feature(ref_065) * 100

        valid_mask = ref_std.prepare_valid_mask(ref_065, dem, sft, sun_zen, coastal_mask, space_mask)
    ...
    cloudy_da_fea_total.to_hdf5('%s_60.h5' % ref_std.short_name, '/%s/cloudy' % csft)

    clear_da_fea_total.to_hdf5('%s_60.h5' % ref_std.short_name, '/%s/clear' % csft)
    ...
    ref_std_lut_ds_x.prior_yes.data[idx] = prior_yes
    ref_std_lut_ds_x.bin_start.data[idx] = bins1[0]
    ref_std_lut_ds_x.bin_end.data[idx] = bins1[-1]
    ref_std_lut_ds_x.delta_bin.data[idx] = bins1[1] - bins1[0]
    ref_std_lut_ds_x.bins.data[idx, :] = bins1[1:]
    ref_std_lut_ds_x.class_cond_ratio_reg.data[idx, :] = ratio

ref_std_lut_ds_x.to_netcdf(r'D:\WorkSpace\20200429\project\data\LUT\Ref_Std_60.nc')
